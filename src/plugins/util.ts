...
const util = {
    ...
    getProjectPath(document: vscode.TextDocument | null) {
        ...
        workspaceFolders && workspaceFolders.forEach(folder => {
     ...
     */
    findStrInFile: function (filePath: string, reg: RegExp) {
    ...
    checkVersion: function (version1:string, version2:string) {
        const v1 = parseInt(version1.replace(/\./g, ''));
        const v2 = parseInt(version2.replace(/\./g, ''));
        return v1 > v2;
    },
    /**
     * 获取某个扩展文件绝对路径
    ...
    openJarByJdGui: function (jarPath:string) {
        // 如何选中文件有待完善
        const jdGuiPath = vscode.workspace.getConfiguration().get('eggHelper.jdGuiPath') as any;
        if (!jdGuiPath) {
